...
// wieder angezeigt; leer lassen heißt behalten.
...
function ready(app, brand) {
  if (app.key === "steam") return true;
...
export function PlatformLinkSettings({ brand, setBrandField, saving, onSave, onClearSecret }) {
  ...
  const runCheck = async (platform) => {
    ...
    try {
      ...
      if (data?.ok) toast.success(`${PLATFORM_APPS.find((p) => p.key === platform)?.label || platform}: alles passt.`);
    } catch (error) {
  ...
  const readyCount = PLATFORM_APPS.filter((app) => ready(app, brand)).length;
    ...
    <div className="border border-white/10 bg-[#121212] rounded-sm p-5 space-y-4" data-testid="platform-link-settings">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-white/50 mt-1">Mitglieder melden sich im Profil einmal bei der Plattform an – der Eintrag wird befüllt und als „verifiziert“ markiert. Dafür braucht die Website je Plattform eine eigene App (Client ID + Secret). Steam braucht keine; PlayStation, Nintendo und EA bieten keine Anmeldung für Websites, Instagram nur für Business-Konten über eine geprüfte Meta-App.</p>
        </div>
      ...
      <div className="grid gap-3">
        {PLATFORM_APPS.map((app) => {
          const isReady = ready(app, brand);
          return (
            <div key={app.key} className="border border-white/10 rounded-sm p-3 space-y-2" data-testid={`platform-app-${app.key}`}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => runCheck(app.key)} disabled={!!checking} data-testid={`platform-check-${app.key}`} className="px-3 py-1 border border-white/20 text-white/80 rounded-sm text-[10px] font-bold uppercase tracking-wider hover:border-[#29B6E8]/60 hover:text-[#29B6E8] disabled:opacity-40">
                    {checking === app.key ? "Prüfe …" : "prüfen"}
                  </button>
                </div>
              </div>
              ...
              <CheckResult platform={app.key} result={checks[app.key]} />
              <SetupGuide guideKey={app.guideKey} />
            </div>
          );
        })}
      </div>
